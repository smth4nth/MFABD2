...
#   inner_v_min  (int)   排除背景的亮度下限 V（0-255），默认 50。
#                        本质是"背景排除阈值"而非"白色最低亮度"：
#                        纯黑背景设 30-50；深色背景设 50-80；浅色背景适当调高。
...
#
# 原理：
#   在红色 blob 的包围框内，查找满足以下三个条件的像素：
#     ① ~red_mask        → 不是红色像素（感叹号区域本身不是红）
#     ② V >= inner_v_min → 亮度高于背景（排除黑色背景）
#     ③ S <= inner_s_max → 饱和度低（排除偏彩色/偏红的内部像素）
#
#   注意：inner_v_min 的本质是"背景排除阈值"，不是"感叹号白色的最低亮度"。
#   游戏渲染会压缩小尺寸元素的亮度，感叹号底部圆点可能只有 V=60-80。
#
# 调参方法：
#   在游戏截图上拾取感叹号内部像素的 RGB（取竖线中段 + 底部圆点各几点），
#   转 HSV，记录 V 最小值和 S 最大值。
#   inner_v_min 设为：min(感叹号各点V) - 10，但不能低于 background_V + 10。
#   inner_s_max 设为：max(感叹号各点S) + 10，通常 < 90 就够。
#
#   同时拾取背景像素 RGB（roi 里非红非感叹号区域），
#   inner_v_min 必须 > 背景 V，否则背景角落像素会混入。
#
# 判断：
...
# inner_blob 只有 1-2 点 → 降 inner_v_min（背景是黑色可低至 30）
# 投影日志无零行     → 继续降 inner_v_min
...
class RedDotDetector(CustomRecognition):
    ...
    def _has_exclamation(self, inner_bright: np.ndarray, gap_ratio: float, debug: bool = False) -> bool:
        ...
        双段存在性只要求各有至少 1 个非零行，不设最小像素数，兼容底部圆点极小的情况。
        """
